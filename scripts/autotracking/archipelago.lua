...
-- Sérialise une table Lua en string lisible (pour le debug)
...
-- Appelé lors de la connexion AP avec les données du slot
-- Adapter selon les options que le mod LiesOfAP envoie via slot_data
function apply_slot_data(slot_data)
    if AUTOTRACKER_ENABLE_DEBUG_LOGGING_AP then
        print(string.format("[LOP] apply_slot_data: %s", dump_table(slot_data)))
    end
    -- TODO: traiter les options du slot (goal, DLC, etc.)
end

lastLevel = "Krat Central Station"

-- Corrigé : le nom de la variable avait un double T (HINT_STTATUS → HINT_STATUS)
HINT_STATUS_MAPPING = {}
if Highlight then
    HINT_STATUS_MAPPING = {
        [20] = Highlight.Avoid,
...
function onClear(slot_data)
	...
	-- [LOP] Décommenter si le mod LiesOfAP envoie des flags via data storage
	-- ap_flags = Archipelago.PlayerNumber.."-lop-flags"
	-- Archipelago:SetNotify({ap_flags})
	-- Archipelago:Get({ap_flags})

	-- [LOP] Décommenter si le mod LiesOfAP envoie la zone courante du joueur
	ap_autotab = Archipelago.PlayerNumber.."-lop-currentLocation"
...
function onDataStorageUpdate(key, value, oldValue)
	...
	-- [LOP] Système de flags non implémenté pour Lies of P (spécifique à Clair Obscur)
	-- if key == ap_flags then ... end

	-- [LOP] Système d'auto-tab non implémenté pour Lies of P
	-- if key == ap_autotab then ... end
end

-- [LOP] mapFlagLocation et printFlagMap sont spécifiques à Clair Obscur → non utilisées
